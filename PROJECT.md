...
- **Fixed in M15 (uncommitted):** Meeting prep refused products chat answered from. Cause: M10's `PRODUCT_ANCHOR_MIN_SCORE` pre-gate searched the BARE product name at 0.35 — a *higher* bar than chat's 0.2, on a degenerate one-word query, at top_k=1 — and vetoed before the real queries ran. The per-query loop was already identical to chat's, so the gate was the only thing that could make prep find nothing while chat found plenty. Replaced with a lexical coverage check on what retrieval returned (`_naming_passage`): retrieval is now exactly chat's, and the Zorblax guard survives because a Cardovex label never contains the word "Zorblax" (re-measured: absent products score 0.3766–0.3975 on generic label vocabulary, i.e. they DO clear 0.2 — the guard is still necessary). Two distinct refusal messages now: empty retrieval vs. name mismatch. NOT REPRODUCED: I could not get a bare-name score under 0.35 in synthetic documents (0.6906 name-dense, 0.6457 name-sparse), so the exact trigger in the real document is unconfirmed; the diagnosis is structural. If prep still refuses, the "none mention X" message names the closest document — widen `_product_terms` to the wording that document uses.
- **Blockers:** Render free tier (512 MB) OOMs on upload. MEASURED ROOT CAUSE: each `pymupdf4llm.to_markdown()` call transiently allocates ~200 MB of working set, and it is **flat in document size** — a 10-page PDF costs the same as a 100-page one (199.6 MB vs 204.2 MB). On top of a ~230 MB idle process that peaks ~445 MB. Tried and REJECTED (measured, no benefit): per-page extraction, page-group extraction, close/reopen the Document per group, and a startup warm-up (warm-up made it worse — +68 MB permanent floor, identical 447 MB peak). Code changes cannot fix this. Real options: (a) bigger instance — 1 GB clears it comfortably; (b) cap accepted page count; (c) move ingest to a separate worker/queue so the spike is not on the web process.
